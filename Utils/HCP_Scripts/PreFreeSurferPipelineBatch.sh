...
#   --Subject=    - quoted, space separated list of subject IDs on which
#                    to run the pipeline
...
main()
{
	...
	StudyFolder="/project/proj_shibusawa/kimura/2021IK01/data" # Location of Subject folders (named by subjectID)
	...
	for Subject in $Subjlist ; do
		...
		# T1w images
		numT1ws=`ls ${StudyFolder}/${Subject}/RawData/NIFTI/*_T1_MPR.nii | wc -l`
		echo "Found ${numT1ws} T1w Images for subject ${Subject}"
		T1wInputImages=`ls ${StudyFolder}/${Subject}/RawData/NIFTI/*_T1_MPR.nii`
		#i=1
		#while [ $i -le $numT1ws ] ; do
		#	T1wInputImages=`echo "${T1wInputImages}${StudyFolder}/${Subject}/RawData/NIFTI/@"`
	        #		i=$(($i+1))
		#done

		# Detect Number of T2w Images and build list of full paths to
		# T2w images
		numT2ws=`ls ${StudyFolder}/${Subject}/RawData/NIFTI/*_T2_SPC.nii | wc -l`
		echo "Found ${numT2ws} T2w Images for subject ${Subject}"
		T2wInputImages=`ls ${StudyFolder}/${Subject}/RawData/NIFTI/*_T2_SPC.nii`
		#i=1
		...
		AvgrdcSTRING="NONE" # since we get out the scanner it is unuseful to use the SE-fieldmap before that...
