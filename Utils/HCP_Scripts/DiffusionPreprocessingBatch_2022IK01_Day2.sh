...
StudyFolder="/home/ikimura/work/KS/KS" #Location of Subject folders (named by subjectID)
Subjlist="KS001MY" #Space delimited list of subject IDs
...
for Subject in $Subjlist ; do
  ...
  RawDataDir="${StudyFolder}/${SubjectID}/RawData_${type}/NIFTI" #Folder where unprocessed diffusion data are
  ...
  PosData=`ls ${RawDataDir}/*_DTI${sess}_33dir_*AP*.nii`
  NegData=`ls ${RawDataDir}/*_DTI${sess}_33dir_*PA*.nii`
  ...
  ${queuing_command} ${HCPPIPEDIR}/DiffusionPreprocessing/DiffPreprocPipeline.sh \
      ...
      --gdcoeffs="${Gdcoeffs}" \
      --printcom=$PRINTCOM --extra-eddy-arg=--repol --extra-eddy-arg=--ol_type=both --extra-eddy-arg=--mb=3 --extra-eddy-arg=--mb_offs=-1 --extra-eddy-arg=--mporder=6 --extra-eddy-arg=--s2v_niter=8 --extra-eddy-arg=--s2v_lambda=5 --extra-eddy-arg=--s2v_interp=trilinear --dwiname=${odir} --combine-data-flag=2
