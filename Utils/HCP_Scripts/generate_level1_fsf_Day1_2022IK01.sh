...
main() {
    ...
    fsf_template_file=${templatedir}/${taskname}_hp160_s4_level1.fsf

    # copy the template file to the intended destination FSF file
    cp -p ${fsf_template_file} ${outdir}/${taskname}_hp160_s4_level1.fsf

    # modify the destination by putting in the correct number of time points
    sed -i "s/fmri(npts) [0-9]*/fmri(npts) ${FMRI_NPTS}/" ${outdir}/${taskname}_hp160_s4_level1.fsf

    echo ""
    echo "Level 1 FSF file generated at: "
    echo "  ${outdir}/${taskname}_hp160_s4_level1.fsf"
    echo ""
}
